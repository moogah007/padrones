...
class AccountExportWizard(models.TransientModel):
    ...
    def doit(self):
        ...
        apgs.write({'txt_export_date': datetime.now()})
        tstr += "PC"
        tstr += "T"
        ...
        if self.export_type_id.payment_form_id.account_bank_export:
            tstr += "{:0>14}".format(self.export_type_id.payment_form_id.account_bank_export)
        else:
            tstr += "{:0>14}".format('0')
        ...
        for apg in apgs:
            for payment in apg.payment_ids:
                if payment.journal_id.id == self.export_type_id.payment_form_id.id:
                    ...
                    tstr += '001'
                    bank = self.env['res.partner.bank'].search([('partner_id', '=', apg.partner_id.id)],limit=1)
                    cbu = bank.cbu
                    tstr += "{:>22}".format(cbu)
                    tstr += datetime.now().strftime('%d%m%Y')
