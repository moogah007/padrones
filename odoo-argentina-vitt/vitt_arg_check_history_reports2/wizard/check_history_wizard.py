...
class AccountCheckHistory(models.Model):
    ...
    date_from = fields.Date(string='Date From', required=True,
        default=datetime.now().strftime('%Y-%m-01'),translate=True)
    date_to = fields.Date(string='Date To', required=True,
        default=str(datetime.now() + relativedelta.relativedelta(months=+1, day=1, days=-1))[:10],translate=True)
    partner_id = fields.Many2many('res.partner',string='Partner',translate=True)
    bank_id = fields.Many2one('res.bank', string='Bank',translate=True)
    ...
    partner_id2 = fields.Many2many('res.partner',string='Partner',translate=True)
    ...
    def ex_checkhist_report(self):
        ...
        return self.env['report'].with_context(landscape=True).get_action(self, 'vitt_arg_check_history_reports2.check_rep', data=datas)

class report_vitt_arg_check_history_reports_check_rep(models.Model):
    _name = "report.vitt_arg_check_history_reports2.check_rep"
    ...
    def render_html(self,docids, data=None):
        ...
        if data['type'] == 'issue_check':
            ...
            report = report_obj._get_report_from_name('vitt_arg_check_history_reports2.check_rep')
            docargs = {
                'docs': checks_op,
                'mode': data['mode'],
                'model': "account.check",
            ...
            if data['partner_id']:
                if data['type'] == 'third_check':
                    domain.append(('check_third_id.partner_id.id', 'in', data['partner_id']))
            if data['bank_id']:
                domain.append(('check_third_id.bank_id.id', 'in', [data['bank_id']]))
            if data['type'] == 'third_check':
                domain.append(('check_third_id.type', '!=', 'issue_check'))
                domain.append(('operation', 'in', data['third_state'] or []))

            checks_op = self.env['account.check.third.operation'].search(domain)

            print checks_op
            sort_data = data['sort_order']
            if sort_data == 'check_nr':
                checks_op = checks_op.sorted(key=lambda r: r.check_third_id.number)
            if sort_data == 'partner':
                checks_op = checks_op.sorted(key=lambda r: r.check_third_id.partner_id.name)
            if sort_data == 'payment_date':
                checks_op = checks_op.sorted(key=lambda r: r.check_third_id.payment_date)

            if data['mode'] == 'overview':
                subsdata = self.env['account.check.third.operation']
                for op in checks_op:
                    for op_f in checks_op:
                        if op.operation == op_f.operation and op.date <= op_f.date \
                                and op.check_third_id.number == op_f.check_third_id.number and op.id != op_f.id:
                            subsdata += op
                checks_op -= subsdata
            ...
            docargs = {
                'docs': checks_op,
                'mode': data['mode'],
                'model': "account.check.third",
                'datat': data['third_state'],
                'datai': data['issue_state'],
                'type': data['type'],
            }
        return self.env['report'].render('vitt_arg_check_history_reports2.check_rep', docargs)
...
class AccountCheckOperation(models.Model):
    _inherit = 'account.check.third.operation'
        ...
        for checkop in docs:
            if data=='camount_total':
                if checkop.operation in datat or []:
                    totval = checkop.check_third_id.amount
            total += totval
        #total = float(MultiplybyRate(checkop.check_id.currency_rate, total, checkop.check_id.company_currency_id, checkop.check_id.currency_id))
        return Decimal(total).quantize(TWOPLACES)
