...
class ReportSlLedger(models.TransientModel):
    ...
    def get_filter(self):
        ...
        return {
            ...
            #'ex_rate_option': self.ex_rate_option,
    ...
    def get_xls_domain(self):
        ...
        #if hasattr(self, 'objected'):
        #    if self.objected:
        #        domain.append(('obj_bool', '!=', False))
        return domain

    def get_xls_domain2(self):
    ...
    def ledger_sl_report_xls(self):
        context = self._context
        filename = 'Sale_ledger.xls'
        ...
        partners_ = invoices.filtered(lambda x: x.payment_move_line_ids != False).mapped('partner_id.id')
        partners = self.env['res.partner'].search([('id','in',partners_)],order="name").mapped("name")
        partners1 = self.env['res.partner'].search([('id','in',partners_)],order="name")
...
class report_vitt_sales_purch_ledger_html_report_pl_ledger(models.Model):
    ...
    def render_html(self,docids, data=None):
        ...
        invoiceModel = self.env['account.invoice']
        if data['sort'] == 'reg_date':
            invoices = invoiceModel.search(domain,order="partner_id,date_invoice")
        else:
            invoices = invoiceModel.search(domain, order="partner_id,date_due")
...
class report_vitt_sales_purch_ledger_html_report_sl_ledger(models.Model):
    ...
    def render_html(self,docids, data=None):
        ...
        invoiceModel = self.env['account.invoice']
        if data['sort'] == 'reg_date':
            invoices = invoiceModel.search(domain,order="partner_id,currency_id,date_invoice")
        else:
            invoices = invoiceModel.search(domain, order="partner_id,currency_id,date_due")
